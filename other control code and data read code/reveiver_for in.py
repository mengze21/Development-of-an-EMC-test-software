...
for i in range(30,1000,100):
    ...
    #print(stop)
    gpib_inst.write('SWE:SPAC LIN')
    #gpib_inst.write('FREQ:STAR %s MHz',start)  # start frequency of display range
    #gpib_inst.write('FREQ:STOP %s MHz',stop)  # stop frequency of display range (not useful)
...
real= M+Antena-kabel11-kabel12-20
...
file=open('receiver_mode_20db.txt','w')
for i in z:
    file.write(str(i)+'\n')
file.close()
...
file=open('receiver_mode_20db_rohdaten.txt','w')
for i in b:
    file.write(str(i)+'\n')
file.close()