...
gpib_inst.write('DET1:REC QPE')                # detector2 average; QPE: Quasipeak
...
for t in range(0,2,1):
    ...
    #print(stop)
            ...
            gpib_inst.write('FREQ:STOP %s MHz'% str(stop))  # stop frequency of display range
    ...
    #print(magnitude)
        ...
        for i in range(1000, 3000, 100):
            ...
            gpib_inst.write('SCAN1:STOP %s MHz;*WAI' % str(stop))  # entry of stop frequency
            # gpib_inst.write('SCAN1:STEP AUTO')
            gpib_inst.write('SCAN1:STEP 1000 kHz')  # entry of step size
            gpib_inst.write(':BAND:TYPE PULS')  # 6 dB??
...
file=open('receiver_mode_10db.txt','w')
for i in z:
    file.write(str(i)+'\n')
file.close()
...
file=open('Python-Max-H_rohdaten_0 dB_2 mal.txt','w')
for i in b:
    file.write(str(i)+'\n')
file.close()